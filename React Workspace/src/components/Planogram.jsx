...
const Planogram = ({ products, locations, planogram }) => {
  ...
  useEffect(() => {
    const calculateMaxDimensions = () => {
      if (parentRef.current) {
        const parentRect = parentRef.current.getBoundingClientRect();
        setMaxDimensions({ width: parentRect.width, height: parentRect.height });
      }
    };
    ...
    <div ref={parentRef} style={{ 
      ...
      {hoveredProduct && (
        <div
          ...
          style={{ 
            top: `${hoveredProduct.position.top}px`, 
            left: `${hoveredProduct.position.left}px` 
          }}
        >
          <p><strong>Name:</strong> {hoveredProduct.name}</p>
          <p><strong>Height:</strong> {hoveredProduct.height}</p>
          <p><strong>Width:</strong> {hoveredProduct.breadth}</p>
          <p><strong>Shelf:</strong> {hoveredProduct.productRow}</p>
          <p><strong>Section:</strong> {hoveredProduct.productSection}</p>
        </div>
      )}
    </div>
  );
